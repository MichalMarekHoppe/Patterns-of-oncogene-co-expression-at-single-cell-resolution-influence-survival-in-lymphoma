...
# 
# MYC-BCL2-BCL6_spatial_sample.txt -  sample data from three high-power 
# microscopic images of DLBCL cases. input data consists of
# a) id - sample id
# b) label - cellular pheotype. every cell has to belong to a group. 
#    In this work, CD20+ cells have been phenotyped into subpopulations 
#    depending on their MYC, BCL2, BCL6 oncogene expression status. CD20 
#    negatiove cells constistute all non-B cells in the analyzed image.
# d) x and y - spatial coordiantes for each cell.
# 
# Number of k neighbours need to can be defined at the beginning of the process.
# 
# Summary results sheet provides results for interactions among all phenotypes 
# for each sample. 
# 
...
library(RANN) 
# import data
data <- read.delim("MYC-BCL2-BCL6_spatial_sample.txt",
                   sep = "\t")
...
for (i in seq_along(id)){
  ...
  nearest_id <- nn2(result_id[,c("x", "y")],
                    result_id[,c("x", "y")],
